      ...
      <v-data-table
        ...
        <template v-slot:top>
          <v-toolbar
            ...
            <v-toolbar-title class="body-1 white--text">Informes de Artículos</v-toolbar-title>
...
//    },
  ...
  created () {
    debugger
    this.items.push(
      {informe: 'ARTICULOS DATOS PRINCIPALES'},
      )
      debugger
      this.sucLis = []
      this.sucSel = []
  ...
  methods: {
    ...
    selInforme(item, row) {
      ...
      this.dialog = true
      if (this.informeTitulo=='ARTICULOS DATOS PRINCIPALES') {
        this.op = 1
      }
    },
    cerrarDialog() {
    ...
    generarInforme(item, row) {
      ...
      if (this.informeTitulo=='ARTICULOS DATOS PRINCIPALES') {
        //this.sayInf = true
        this.headersRes = [{ title: 'Codigo', dataKey:'codigo' },{ title: 'Nombre', dataKey:'nombre' }]
        let v = this.$store.state.vinculosPadres
        return HTTP().post('/infart_datosprincipales', { vinculos: v })
        .then((data) => {
          debugger
          this.rows = []
    ...
    print(informeTitulo, headersRes, itemRes, orientacion ) {
      ...
      //doc.autoPrint();
      //doc.output('dataurlnewwindow', {filename: 'informe.pdf'});
