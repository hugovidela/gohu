    ...
    <v-flex>
      <v-data-table
        ...
        <template v-slot:item.nombre="{ item }">
          ...
          <v-badge
            class="pt-3"
            :content="item.tagsprecios.length ? item.tagsprecios.length : '0'"
            :color="temas.forms_btn_add_bg"
...
export default {
  ...
  data: () => ({
    ...
    headersArticulos: [
      { text: 'ID', value: 'precio.articulo.codigo', align: 'left', width: 90, sortable: false, },
      { text: 'NOMBRE', value:'precio.articulo.nombre', align: 'left', width: 400},
      { text: 'ACCIONES', value: 'accion', sortable: false },
    ],
    editedIndex: -1,
    editado: {
  ...
  methods: {
   ...mapMutations(['alert','closeAlert']),
    ...
    buscoNombre (event) {
      return HTTP().post('/preciotagexiste', {nombre: this.editado.nombre,id: this.editado.id}).then(({ data }) => {
        if(data) {
          this.datosOk = false
    ...
    verReglas(item) {
      return HTTP().post('/puedoborrartagarticulos', {id: item.id}).then(({ data }) => {
        debugger
        let m = ''
    ...
    preguntoBorrar (item) {
      return HTTP().post('/puedoborrartagarticulos', {id: item.id}).then(({ data }) => {
        ...
        if (data.res.length==0) {
          ...
          let m = ''
          if (data.tagsPrecioUser.length>0) {
            m = 'Este tag esta vinculado a '
            if (data.tagsPrecioUser.length>1) {
              m += data.tagsPrecioUser.length+' artículos.<br>'
              m += 'Si lo elimina se desvincuará en forma automatica de ellos.<br>'
            } else {
              m += data.tagsPrecioUser.length+' artículo.<br>'
              m += 'Si lo elimina se desvincuará en forma automatica.<br>'
            }
          }
